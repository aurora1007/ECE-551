...
board_t * makeBoard(int w, int h, int numMines) {
  //WRITE ME!
  board_t * block = malloc(sizeof(*block));
  ...
  block->width = w;
  block->height = h;
  block->totalMines = numMines;
  block->board = malloc(w * sizeof(block->board));

  if (block->board == NULL) {
    fprintf(stderr, "2D Board Allocation Failed\n");
    exit(EXIT_FAILURE);
  }
  for (int y = 0; y < h; y++) {
    block->board[y] = malloc(h * sizeof(block->board[y]));

    if (block->board[y] == NULL) {
      fprintf(stderr, "2D Board Allocation failed\n");
      exit(EXIT_FAILURE);
    }
    for (int x = 0; x < w; x++) {
      block->board[y][x] = UNKNOWN;
    }
  }
